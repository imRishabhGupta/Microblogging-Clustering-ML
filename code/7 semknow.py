# -*- coding: utf-8 -*-
"""
Created on Sat Oct 15 22:24:09 2016
...
with open('semantics_phrases_wiki.txt', 'r') as f:
   features_wiki_phrases=pickle.load(f)

features_wiki_word=[]

with open('semantics_words_wiki.txt', 'r') as f:
   features_wiki_word=pickle.load(f)

features=terms + features_wiki_word + semantic_features_word + features_wiki_phrases
features = rem_reduncy(features)

tfidf_vectorizer = TfidfVectorizer( max_features=200000, stop_words='english', ngram_range=(1, 2),
                                 vocabulary=features, use_idf=True, tokenizer=tokenize_and_stem)

tfidf_matrix = tfidf_vectorizer.fit_transform(tweets)

num_clusters = 100
km = KMeans(n_clusters=num_clusters,  max_iter=200, n_init=20)
km.fit(tfidf_matrix)
tf=time.time()
te=tf-ts
